...
if membro_selecionado:
    ...
    # Criar cores diferentes para blocos consecutivos
    cores = []
    last_status = None
    for s in df_membro["status_simples"]:
        if s != last_status:
            color = "green" if s=="P" else "red"
            last_status = s
        cores.append(color)

    st.subheader(f"📈 Gráfico de Presenças: {membro_selecionado}")
    fig, ax = plt.subplots(figsize=(12,4))
    ax.bar(df_membro["ensaio_data"], df_membro["valor"], color=cores, width=0.8)
    ax.set_ylim(0,1.1)
    ax.set_yticks([0,1])
    ax.set_yticklabels(["Falta", "Presença"])
    ax.set_xlabel("Data do Ensaio")
...
st.markdown("---")
st.markdown("✅ Cada barra representa um ensaio. Verde = presença, Vermelho = falta, cores mudam a cada bloco consecutivo.")

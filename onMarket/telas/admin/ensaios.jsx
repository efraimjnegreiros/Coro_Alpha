...
export default function EnsaiosADM() {
  ...
  const [musicas, setMusicas] = useState(''); // novo campo
  ...
  async function carregarEnsaios() {
    try {
      setCarregando(true);
      const resposta = await axios.get('https://coro-alpha.onrender.com/api/ensaios');
  ...
  function abrirModalEditar(ensaio) {
    ...
    setMusicas(Array.isArray(ensaio.musicas) ? ensaio.musicas.join(', ') : ensaio.musicas || '');
    setModoEdicao(true);
    setModalVisivel(true);
  }
  ...
  async function salvarEnsaio() {
    ...
    const musicasArray = musicas
      ? musicas.split(',').map((m) => m.trim()).filter((m) => m)
      : [];

    try {
      if (modoEdicao) {
        await axios.put(`https://coro-alpha.onrender.com/api/ensaios/${ensaioSelecionado.id}`, {
          descricao,
          data: dataISO,
          hora,
          local,
        ...
        await axios.post('https://coro-alpha.onrender.com/api/ensaios', {
          descricao,
          data: dataISO,
          hora,
          local,
      ...
      console.error('Erro ao salvar ensaio:', error.response || error.message);
  ...
  async function deletarEnsaio() {
    ...
    try {
      await axios.delete(`https://coro-alpha.onrender.com/api/ensaios/${ensaioSelecionado.id}`);
          ...
          <View style={estilos.lista}>
            {ensaiosFiltrados.map((ensaio) => (
              ...
              >
                ...
                {ensaio.musicas && ensaio.musicas.length > 0 && (
                  <Text style={estilos.info}>
                    🎵 Músicas: {Array.isArray(ensaio.musicas) ? ensaio.musicas.join(', ') : ensaio.musicas}
                  </Text>
                )}
              </TouchableOpacity>
            ))}
      ...
      <Modal visible={modalVisivel} transparent animationType="slide">
        <View style={estilos.modalContainer}>
          <View style={estilos.modalConteudo}>
            ...
            <TextInput
              style={estilos.input}
              placeholder="Músicas (separe por vírgulas)"
              placeholderTextColor="#aaa"
              value={musicas}
              onChangeText={setMusicas}
            />

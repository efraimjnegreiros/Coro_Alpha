...
export default function InicioADM() {
  ...
  function atualizarAniversariantes() {
    ...
    const aniversariantesDoMes = membros.filter((m) => {
      ...
      const [ano, mes, dia] = m.dataNascimento.split('-').map(Number); // YYYY-MM-DD
        ...
        {aniversariantes.length > 0 && (
          <View style={{ marginBottom: 20 }}>
            ...
            {aniversariantes.map((m) => {
              const dia = new Date(m.dataNascimento).getDate();
              return (
                <Text key={m.id} style={{ fontSize: 14, color: '#444', marginBottom: 5 }}>
                  {m.nome} – {String(dia).padStart(2, '0')}
                </Text>
              );
            })}
          </View>
